...
// extern = external (telling the compiler that this variable exists somewhere else, prob another .c file)
// const pointer* = the data being pointed can't change
// pointer* const = the pointer* itself can't be reassigned
extern
const IrCode *
  const NApowerCodes[];
extern
...
// const IrCode = IrCode as read-only
// powerCode is a pointer, pointing to IrCode (we got two, NA and EU powerCodes)
...
uint8_t read_bits(uint8_t count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (bitsleft_r == 0) {
      ...
      bits_r = powerCode -> codes[code_ptr++]; // powerCode is a pointer to IrCode, -> = access a member of a struct being pointed at
      DEBUGP(putstring("\n\rGet byte: "); // macro from main.h, if DEBUGP == 1: execute the code within
        putnum_uh(bits_r);
      );
...
uint16_t ontime, offtime; // 0-65535
uint8_t i, num_codes;
uint8_t region;
...
void sendAllCodes(const IrCode *
  const sel_powerCode[], uint8_t num_codes) {
  ...
  for (uint8_t i = 0; i < num_codes; i++) {
    ...
    for (uint8_t k = 0; k < numpairs; k++) {
      ...
      DEBUGP(putstring("\n\rti = "); putnum_ud(ti >> 1); putstring("\tPair = "); putnum_ud(ontime));
...
void loop() {
  ...
  if (digitalRead(TRIGGER) == BUTTON_PRESSED) {
    ...
    DEBUGP(putstring_nl("( ͡° ʖ̯ ͡°)=E/̵͇̿̿/’̿̿ ̿ ̿̿ ̿ ̿  - Ayo, dont ya dare touch that TV again, cuh!"));
    sendAllCodes(EUpowerCodes, num_EUcodes);
    DEBUGP(putstring_nl("ʕ ͡° ʖ̯ ͡°ʔ - Oi, would you kindly stop fiddlin with the telly, mate? Some of us are tryin to watch!"));
  }
  yield();
}
